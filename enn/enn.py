...
class ENN:
    ...
    def __init__(self, torch_model, numberofensemble):
        ...
        # self.param_index: sotre the location of parameters in a 1-d list. e.g. [[1,3],[3,5],[5,9], the [1,3] shows that the first parameters start from 1 and end at 3 in the 1-d list
    ...
    def add_var(self, value):
        ...
        value = 0.99 * value + 0.01 * value_mean + value_std * 1 * torch.randn(self.Nw, self.Ne) #调整W的地方，比如value_std * 0.5
        #value = value + 0.5 * (value - value_mean) + value_std * 1 * torch.randn(self.Nw, self.Ne)
        #value = value * (1 + 0.03 * torch.randn(self.Nw, self.Ne))
        return value
